...
public class Auto extends SubCommand {
	...
	public boolean onCommand(final PlotPlayer player, String[] args) {
		...
		if (player.getMeta(Auto.class.getName(), false)
				|| !checkAllowedPlots(player, plotarea, allowed_plots, size_x, size_z))
			return false;
		...
		final Collection<Plot> checkplot = plotarea.getPlots();

		if (checkplot != null) {

			for (Plot plot : checkplot) {

				com.intellectualcrafters.plot.object.Location loc = plot.getCenter();
				...
				if (loc.getX() <= (center.getX() - radius + 1000) || loc.getX() >= (center.getX() + radius - 1000)
						|| loc.getZ() <= (center.getZ() - radius + 1000)
						|| loc.getZ() >= (center.getZ() + radius - 1000)) {

					player.sendMessage(
							"§b§l[ §f§lKite SV §b§l] §c세계 경계 밖입니다. 좌표: x:" + loc.getX() + " z:" + loc.getZ());
		...
		if (EconHandler.getEconHandler() != null && plotarea.USE_ECONOMY) {
			...
			double cost = costExp.evaluate(
					(double) (Settings.Limit.GLOBAL ? player.getPlotCount() : player.getPlotCount(plotarea.worldname)));
	...
	private static boolean checkAllowedPlots(PlotPlayer player, PlotArea plotarea, @Nullable Integer allowed_plots,
	 ...
	 */
	public static void autoClaimSafe(final PlotPlayer player, final PlotArea area, PlotId start, final String schem,
			@Nullable final Integer allowed_plots) {
	...
	public static void autoClaimFromDatabase(final PlotPlayer player, final PlotArea area, PlotId start,
			final RunnableVal<Plot> whenDone) {
